<script lang="ts">
    ...
    async function handleLogin(e: Event) {
        ...
        try {
            const res = await fetchWithAuth('http://localhost:8000/api/v1/login', {
                method: 'POST',
            ...
            if (!res.ok) {
                errorMessage = data.message || 'Login gagal. Periksa kembali email dan password Anda.';
                return;
            }
...
<div class="min-h-screen flex items-center justify-center p-4 lg:p-8 animate-fade-in relative overflow-hidden">
    ...
    <div class="w-full max-w-5xl bg-[#0C2C27] rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.25)] flex flex-col md:flex-row overflow-hidden border border-[#10B981]/10 relative z-10">
        ...
        <div class="w-full md:w-1/2 p-8 lg:p-12 flex flex-col justify-center">
            ...
            <!-- Social Logins -->
            <div class="space-y-3">
                <a href="http://localhost:8000/api/v1/auth/google/redirect" class="w-full flex items-center justify-center gap-3 bg-[#123B35] hover:bg-[#0F312B] text-white py-3.5 rounded-[16px] transition-all border border-white/5 font-medium">
                ...
                <a href="http://localhost:8000/api/v1/auth/apple/redirect" class="w-full flex items-center justify-center gap-3 bg-[#123B35] hover:bg-[#0F312B] text-white py-3.5 rounded-[16px] transition-all border border-white/5 font-medium">
