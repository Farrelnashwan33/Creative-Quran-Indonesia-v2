<script lang="ts">
  ...
  // Accept data from parent layout to get Supabase session
  let { children, data } = $props();

  // Derive Supabase session from layout data (passed from root +layout.ts)
  ...
  // Sync Supabase session into legacy authToken store so the rest of the app
  // (which checks $authToken) keeps working after a Supabase login.
  $effect(() => {
    if (session) {
      // Supabase user is logged in — populate legacy stores
...
<!-- Render layout immediately without waiting for mounted — fixes blank screen after Supabase Auth was added -->
